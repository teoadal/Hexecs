...
var world = new WorldBuilder()
    .UseDefaultParallelWorker(4) // чтобы обрабатывать сущности в 4 потока
    .UseAsDebugWorld() // чтобы получать больше информации в Debug-режиме
    .UseLogger(logger => logger.UseConsoleSink()) // чтобы логи выводились в консоль
    .Build();
```
...
Actor passenger = world.Actors.BuildActor(passengerAsset, Args.Rent("plane", planeActor));
```
