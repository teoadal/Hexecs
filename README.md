...
В HexECS существует два вида систем:
- **Системы обновления** - обновляют состояние актёров в контексте актёров (это системы, которые имплементируют интерфейс `IUpdateSystem`).
- **Системы рисования** - рисуют состояния актёров из контекста актёров (`IDrawSystem`).

Сейчас мы зарегистрируем систему обновления.
Мы сделаем это при создании мира, так как у нас всего один контекст актёров.
...
Мы можем обновить контекст актёра отдельно от остальных контекстов (`actorContext.Update`), но, чтобы это делать, нужно иметь веское обоснование.

Чтобы нарисовать мир, необходимо вызвать другой метод мира `world.Draw`. 
Обратите внимание, что обновление мира заставляет его перейти в состояние `Update`, тогда как рисование переводит мир в состояние `Draw`.
Эти состояния можно проверить в свойстве `world.State`. 
Это состояние нужно учитывать при многопоточной работе приложения, либо не допускать отрисовки (пропускать кадры), пока мир не обновился.
