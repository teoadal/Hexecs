...
internal sealed class Archetype<T1, T2, T3>(ushort id): IArchetype
    ...
    where T3 : struct, IActorComponent
{
    ...
    public bool AddFrom(IArchetype source, uint actorId)
    {
        ...
        ref var existsComponent3 = ref source.TryGetRef<T3>(actorId);
        if (!Unsafe.IsNullRef(ref existsComponent3))
        {
            components.Item3 = existsComponent3;
        }
    ...
    public bool TryGet<T>(uint actorId, out T component) where T : struct, IActorComponent
    {
        ...
        if (!Unsafe.IsNullRef(ref components))
        {
            ...
            {
                component = Unsafe.As<T3, T>(ref components.Item3);
                return true;
            }
        }
    ...
    public ref T TryGetRef<T>(uint actorId) where T : struct, IActorComponent
    {
        ...
        if (!Unsafe.IsNullRef(ref components))
        {
            ...
            if (typeof(T) == typeof(T3))
            {
                return ref Unsafe.As<T3, T>(ref components.Item3);
            }
        }
    ...
    public bool Set<T>(uint actorId, ref readonly T component) where T : struct, IActorComponent
    {
        ...
        if (!Unsafe.IsNullRef(ref components))
        {
            ...
            if (typeof(T) == typeof(T3))
            {
                components.Item3 = Unsafe.As<T, T3>(ref Unsafe.AsRef(in component));
                return true;
            }
        }
