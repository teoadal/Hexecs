...
{
    private static readonly EqualityComparer<ushort> IdComparer = EqualityComparer<ushort>.Default;
    ...
    /// <summary>
    /// Получает ссылку на компонент актёра (<see cref="ActorComponentRef{T}"/>).
    /// </summary>
    /// <typeparam name="T">Тип компонента. Должен быть структурой и реализовывать <see cref="IActorComponent"/>.</typeparam>
    /// <param name="actorId">Идентификатор актёра.</param>
    /// <returns>Ссылка на компонент актёра или <see cref="ActorComponentRef{T}.Empty"/>, если компонент не найден.</returns>
    public ActorComponentRef<T> GetComponentRef<T>(uint actorId) where T : struct, IActorComponent
    {
        var pool = GetComponentPool<T>();
        if (pool == null) return ActorComponentRef<T>.Empty;

        var index = pool.TryGetIndex(actorId);

        return index == -1
            ? ActorComponentRef<T>.Empty
            : new ActorComponentRef<T>(pool, index);
    }
        ...
        where T : struct, IActorComponent
    {
        ...
        ref var entry = ref GetEntryExact(actorId);
        entry.Components.Remove(ActorComponentType<T>.Id, IdComparer);
