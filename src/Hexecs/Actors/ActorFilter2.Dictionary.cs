...
public sealed partial class ActorFilter<T1, T2>
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private void AddEntry(uint actorId, int index1, int index2)
    {
        if (TryAddEntry(actorId, index1, index2)) return;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private bool TryAddEntry(uint actorId, int index1, int index2)
    {
        var pageIndex = (int)(actorId >> PageBits);
        var pages = _sparsePages;
        ...
        if ((uint)pageIndex < (uint)pages.Length)
        {
            ...
            if (page != null && (uint)_count < (uint)_dense.Length)
            {
                ...
                if (slot == 0) // Чистая вставка (самый частый случай в ECS)
                {
                    ...
                    slot = idx + 1;
                    _dense[idx] = actorId;
                    _values[idx] = new Entry(index1, index2);
                    _count++;
        ...
        return TryAddEntrySlow(actorId, index1, index2);
    }

    [MethodImpl(MethodImplOptions.NoInlining)]
    private bool TryAddEntrySlow(uint actorId, int index1, int index2)
    {
        EnsureDenseCapacity();
        var pageIndex = (int)(actorId >> PageBits);
        ...
        _values[denseIndex] = new Entry(index1, index2);
        _count++;
