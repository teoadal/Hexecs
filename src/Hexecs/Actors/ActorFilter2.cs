    ...
    where T2 : struct, IActorComponent
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public bool Contains(in ActorRef<T1, T2> actor) => Contains(actor.Id);
    ...
    private void Add(uint actorId, int index1, int index2)
    {
        ...
        if (_postponedReadersCount == 0) AddEntry(actorId, index1, index2);
        else _postponedUpdates.Enqueue(Operation.Add(actorId, index1, index2));
    }
    ...
    private void ProcessPostponedUpdates()
    {
        ...
        {
            if (operation.IsAdd) AddEntry(operation.Id, operation.Index1, operation.Index2);
