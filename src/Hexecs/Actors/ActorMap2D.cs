    ...
    where T : struct, IActorComponent
{
    ...
    public void Update(bool clearBefore = true)
    {
        ...
        while (poolEnumerator.MoveNext())
        {
            ...
            OnComponentAdded(actor.Id, poolEnumerator.Index, in actor.Component1);
        }
    }
    ...
    private void OnComponentAdded(uint actorId, int index, in T component)
    {
    ...
    private void OnComponentRemoving(uint actorId, in T component)
    {
        var (x, y) = _positionExtractor(component);
        if (x >= _values.Length) return;
    ...
    private void OnComponentUpdating(uint actorId, in T exists, in T expected)
    {
