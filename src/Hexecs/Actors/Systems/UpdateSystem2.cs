...
public abstract class UpdateSystem<T1, T2> : UpdateSystem, IParallelJob
    where T1 : struct, IActorComponent
    where T2 : struct, IActorComponent
{
    public readonly ActorFilter<T1, T2> Filter;
    ...
    public sealed override void Update(in WorldTime time)
    {
        if (Enabled)
        {
            ...
            if (length > 0)
            {
                ...
                {
                    foreach (ActorRef<T1, T2> actor in Filter)
                    {
    ...
    void IParallelJob.Execute(int workerIndex, int workersCount)
    {
        ...
        {
            ActorFilter<T1, T2>.SkipTakeEnumerator batch = workerIndex == workersCount
                ? Filter.Skip(start)
