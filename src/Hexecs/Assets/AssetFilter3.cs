    ...
    where T3 : struct, IAssetComponent
{
    ...
    public AssetRef<T1, T2, T3> Get(AssetId assetId)
    {
        ...
        AssetConstraint? constraint)
    {
        ...
        foreach (Asset asset in context)
        {
            ...
            int index3 = pool3.TryGetIndex(assetId);
            if (index3 == -1)
            {
                continue;
            }
            ...
            ArrayUtils.Insert(
                ...
                bufferPool,
                length,
                new KeyValuePair<uint, Entry>(assetId.Value, new Entry(index1, index2, index3)));
