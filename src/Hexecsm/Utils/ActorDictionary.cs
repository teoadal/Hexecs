...
internal sealed partial class ActorDictionary<TValue>(int initialCapacity)
    where TValue : struct
{
    private const uint EmptySlot = 0;

    public ReadOnlySpan<ActorId> Keys
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        get => new ReadOnlySpan<ActorId>(_dense, 0, _count);
    }
    ...
    public bool TryAdd(ActorId key, in TValue value)
    {
        ...
        return TryAddEntrySlow(key, in value);
    }
    ...
    private bool TryAddEntrySlow(ActorId key, in TValue value)
    {
        uint keyRaw = key.Value;
