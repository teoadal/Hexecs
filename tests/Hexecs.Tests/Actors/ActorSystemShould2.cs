...
public sealed class ActorSystemShould2(ActorTestFixture fixture) : IClassFixture<ActorTestFixture>
{
    ...
    public void UpdateActorsInParallel(int degreeOfParallelism, int actorCount)
    {
        ...
        ActorFilter<Defence, Attack> actorFilter = actorContext.Filter<Defence, Attack>();

        actorFilter.Length
            .Should()
            .Be(actorCount);
        
        foreach (ActorRef<Defence, Attack> actor in actorFilter)
        {
    ...
    private sealed class ParallelUpdateSystem(
        ...
        : UpdateSystem<Defence, Attack>(context, parallelWorker: parallelWorker)
    {
        protected override void Update(in ActorRef<Defence, Attack> actor, in WorldTime time)
        {
