...
public sealed class InlineBucketShould
{
    ...
    public void Contains_WithCustomEqualityComparer_ShouldUseIt()
    {
        ...
        EqualityComparer<TestStruct> comparer = EqualityComparer<TestStruct>.Create((x, y) => x.Value == y.Value, x => x.Value.GetHashCode());
        var searchItem = new TestStruct { Value = 10 }; // Не та же ссылка, но то же значение

        // Act
        bool result = bucket.Contains(searchItem, comparer);
    ...
    public void GetEnumerator_ShouldEnumerateItems()
    {
        ...
        foreach (int item in expectedItems)
        {
            bucket.Add(item);
        }
    ...
    public void GetRef_ShouldReturnReferenceToItem(int index)
    {
        ...
        // Act
        ref int itemRef = ref bucket.GetRef(index);
        int originalValue = itemRef;
        itemRef = 100;
    ...
    public void IndexOf_WithCustomEqualityComparer_ShouldUseIt()
    {
        ...
        EqualityComparer<TestStruct> comparer = EqualityComparer<TestStruct>.Create(
            (x, y) => x.Value == y.Value,
            x => x.Value.GetHashCode());

        // Act
        int index = bucket.IndexOf(new TestStruct { Value = 20 }, comparer);
    ...
    public void Remove_WithCustomEqualityComparer_ShouldUseIt()
    {
        ...
        // Act
        bool result = bucket.Remove(new TestStruct { Value = 20 }, comparer);
