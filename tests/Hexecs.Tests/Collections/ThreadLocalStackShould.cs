...
public sealed class ThreadLocalStackShould
{
    ...
    public void Store_And_Retrieve_Single_Element(uint value)
    {
        ...
        // Act
        stack.Push(value);
        bool success = stack.TryPop(out uint result);
    ...
    public void Return_False_On_Empty_Stack()
    {
        ...
        // Act
        bool success = stack.TryPop(out _);
    ...
    [Fact(
        DisplayName = "Стек должен корректно работать с большим объемом данных в однопоточном режиме, гарантируя отсутствие потерь при миграции данных в глобальный пул")]
    [Trait("Category", "Unit")]
    public void Correctly_Handle_Large_Volume_In_Single_Thread()
    {
        ...
        while (stack.TryPop(out uint result))
        {
            results.Add(result);
        }
        ...
        uint resultHash = results.Aggregate(0u, (s, v) => s ^ v);
        var expectedHash = 0u;
    ...
    public void Ensure_No_Data_Loss_Under_Parallel_Load()
    {
        ...
        var stack = new ThreadLocalStack<uint>(totalCount);
        ...
        Parallel.For(
            ...
            t =>
            {
                for (var i = 0; i < opsPerThread; i++)
                {
                    ...
                    if (stack.TryPop(out uint popped))
                    {
        ...
        while (stack.TryPop(out uint remaining))
        {
    ...
    public async Task Allow_Threads_To_Exchange_Data_Through_Global_Pool()
    {
        ...
        List<uint> results = await Task.Run(() =>
        {
